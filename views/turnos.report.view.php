...
<div class="rp-native-header">
    ...
    <div class="rp-nh-right">
        ...
        <a href="zabbix.php?action=turnos.report.pdf&date=<?= $date ?>&shift=<?= $shift ?>" target="_blank"
...
<div class="rp-kpi-grid">
    <div class="rp-kpi"><div class="rp-kpi-icon bg-blue"><i class="fas fa-bell"></i></div><div class="rp-kpi-body"><span class="rp-kpi-val"><?= (int)$data['totals']['total'] ?></span><span class="rp-kpi-label">Total Eventos</span></div></div>
    <div class="rp-kpi"><div class="rp-kpi-icon bg-red"><i class="fas fa-fire"></i></div><div class="rp-kpi-body"><span class="rp-kpi-val"><?= (int)$data['totals']['critical'] ?></span><span class="rp-kpi-label">Críticos</span></div></div>
    <div class="rp-kpi"><div class="rp-kpi-icon bg-orange"><i class="fas fa-clock"></i></div><div class="rp-kpi-body"><span class="rp-kpi-val"><?= rp_duration($data['global_mtta']) ?></span><span class="rp-kpi-label">MTTA Global</span></div></div>
    <div class="rp-kpi"><div class="rp-kpi-icon bg-yellow"><i class="fas fa-exclamation-circle"></i></div><div class="rp-kpi-body"><span class="rp-kpi-val"><?= count($data['unacked']) ?></span><span class="rp-kpi-label">Sem ACK</span></div></div>
    <div class="rp-kpi"><div class="rp-kpi-icon bg-purple"><i class="fas fa-history"></i></div><div class="rp-kpi-body"><span class="rp-kpi-val"><?= count($data['inherited']) ?></span><span class="rp-kpi-label">Herdados</span></div></div>
    <div class="rp-kpi"><div class="rp-kpi-icon bg-green"><i class="fas fa-users"></i></div><div class="rp-kpi-body"><span class="rp-kpi-val"><?= count($data['presence']) ?></span><span class="rp-kpi-label">Analistas Online</span></div></div>
</div>
...
<div class="rp-charts-row">
    ...
    <div class="rp-card rp-card-chart">
        <div class="rp-card-head"><i class="fas fa-chart-pie"></i> Distribuição por Severidade</div>
...
<div class="rp-noise-row">
    <div class="rp-card">
        <div class="rp-card-head"><i class="fas fa-server"></i> Top 5 Hosts</div>
    ...
    <div class="rp-card">
        <div class="rp-card-head"><i class="fas fa-bolt"></i> Top 5 Triggers</div>
...
<div class="rp-card">
    ...
    <?php else: ?>
        ...
        <tr>
            ...
            <td class="td-bold"><?= (int)$p['online_minutes'] ?> min</td>
        </tr>
        <?php endforeach; ?>
        </tbody></table>
    <?php endif; ?>
...
if (document.getElementById('chartMtta')) {
    new Chart(document.getElementById('chartMtta'), {
        type:'line', data:{labels:MTTA_LABELS, datasets:[{label:'MTTA (s)',data:MTTA_DATA,
            borderColor:'#0275b8',backgroundColor:'rgba(2,117,184,0.08)',borderWidth:2.5,fill:true,tension:0.35,
            pointBackgroundColor:'#0275b8',pointRadius:4,pointHoverRadius:6}]},
        options:{responsive:true,maintainAspectRatio:false,
            plugins:{legend:{display:false},tooltip:{callbacks:{label:function(c){const s=c.parsed.y;
                return s<60?s+'s':s<3600?Math.floor(s/60)+'m '+(s%60)+'s':Math.floor(s/3600)+'h '+Math.floor((s%3600)/60)+'m';}}}},
            scales:{x:{grid:{display:false},ticks:{font:{size:11},color:'#999'}},
                y:{grid:{color:'#f0f0f0'},beginAtZero:true,ticks:{font:{size:11},color:'#999',
...
if (document.getElementById('chartSev')) {
    const sevChart = new Chart(document.getElementById('chartSev'), {
        type:'doughnut', data:{labels:SEV_LABELS, datasets:[{data:SEV_DATA,
            backgroundColor:['#97AAB3','#7499FF','#ffb600','#e97659','#ff4530','#d22824'],borderWidth:2,borderColor:'#fff'}]},
        options:{responsive:true,maintainAspectRatio:false,cutout:'55%',
            plugins:{legend:{position:'right',labels:{font:{size:11},padding:12,usePointStyle:true,pointStyle:'circle',
                generateLabels:function(chart){
                    const d=chart.data; return d.labels.map(function(l,i){
                        return {text:l+' ('+d.datasets[0].data[i]+')',fillStyle:d.datasets[0].backgroundColor[i],
                            strokeStyle:'#fff',lineWidth:2,index:i,hidden:false,pointStyle:'circle'};
                    });
                }
            }},
...
(function(){
    ...
    for (let i = 29; i >= 0; i--) {
        ...
        const border = isSelected ? 'border:2px solid #0275b8;' : '';
...
if (noteForm) {
    noteForm.addEventListener('submit', function(e) {
        ...
        fetch('zabbix.php?action=turnos.report.notes.save', {
            ...
            if(j.success){st.textContent=j.message;st.style.color='#2e7d32';ta.value='';setTimeout(()=>location.reload(),1500);}
